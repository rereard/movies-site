...
const Search = ({title, search, setSearch, setMovies, setPage}) => {
    ...
    return (
        <div className={`flex flex-col md:flex-row ${search ? 'mt-20' : 'mt-10'} w-11/12 justify-center md:justify-between`}>
            ...
            <form className='flex w-full md:items-center md:w-96 rounded-lg flex-col md:flex-row' onSubmit={(event) => {
                ...
                <input type="submit" disabled={searchValue === '' ? true : false} value='Search' className='mt-2 md:mt-0 text-md p-2 cursor-pointer disabled:cursor-default bg-blue-500 disabled:bg-blue-300 rounded-lg sm:rounded-none sm:rounded-r-lg border border-blue-800 disabled:border-blue-400' />
            </form>
        </div>
    )
}
